    ...
    <section className="reveal-section grid gap-8 bg-[#F3F3F3] px-[max(6vw,40px)] py-30 md:grid-cols-[.75fr_1.25fr] md:gap-25">
      <div className="reveal-item">
        ...
        <div className="mt-12 flex gap-2.5">
          <button className="grid h-11 w-11 place-items-center border border-[#0A272329] text-lg transition hover:border-[#0A2723] hover:bg-[#0A2723] hover:text-white" type="button" onClick={() => show(current - 1)} aria-label="Depoimento anterior">←</button>
          <button className="grid h-11 w-11 place-items-center border border-[#0A272329] text-lg transition hover:border-[#0A2723] hover:bg-[#0A2723] hover:text-white" type="button" onClick={() => show(current + 1)} aria-label="Próximo depoimento">→</button>
        </div>
      </div>
      <div className="reveal-item reveal-delay-2 self-center" aria-live="polite">
        <figure className="animate-[fade-quote_.35s_ease] border-l border-[#D1AD7D] pl-7 md:pl-15" key={item.name}>
        ...
        <div className="ml-7 mt-8.5 flex items-center gap-2.25 md:ml-15" aria-label="Paginação dos depoimentos">
          {testimonials.map((testimonial, index) => <button className={`h-0.75 p-0 transition ${index === current ? 'w-10.5 bg-[#D1AD7D]' : 'w-5.5 bg-[#E0C8A0] hover:bg-[#D1AD7D]'}`} type="button" aria-label={`Ver depoimento ${index + 1}`} onClick={() => show(index)} key={testimonial.name} />)}

...
const practiceAreas = [
  { number: '01', symbol: '✦', title: 'Direito de Família e Sucessões', description: 'Apoio cuidadoso em momentos que envolvem patrimônio, vínculos e novos começos.', featured: true },
  { number: '02', symbol: '◌', title: 'Direito Civil', description: 'Proteção de direitos, contratos e relações do dia a dia.' },
  { number: '03', symbol: '⌁', title: 'Direito Imobiliário', description: 'Segurança jurídica para negócios e patrimônio imobiliário.' },
];
    ...
    <section id="atuacao" className="reveal-section bg-[#0A2723] px-5.5 py-20 text-[#f7f4ee] md:px-[max(6vw,40px)] md:py-30">
      ...
      <div className="grid md:grid-cols-2 xl:grid-cols-3">
        {practiceAreas.map((area, index) => (
          <article key={area.title} className={`reveal-item reveal-delay-${index + 1} flex min-h-65 cursor-pointer flex-col justify-between border border-white/20 p-6.75 md:min-h-78.75 ${area.featured ? 'bg-[#D1AD7D]' : 'transition hover:bg-[#233c57]'}`}>
